<script lang="tsx">
  ...
  export default defineComponent({
    ...
    setup(props, { slots }) {
      ...
      // 控制当前组件时候显示 isShow: v-show实现 isIfShow: v-if实现
      function getShow(): { isShow: boolean; isIfShow: boolean } {
        const { show, ifShow } = props.schema
        const { showAdvancedButton } = props.formProps
        const itemIsAdvanced = showAdvancedButton && isBoolean(props.schema.isAdvanced) ? props.schema.isAdvanced : true
      ...
      function handleRules(): ValidationRule[] {
        ...
        if (characterInx !== -1 && !rules[characterInx].validator) {
          rules[characterInx].message = rules[characterInx].message || `字符数应小于${[rules[characterInx].max] as Recordable}位`
        }
        return rules
      }
